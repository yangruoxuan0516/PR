...
double compute_energy(GUIParams& params, const Eigen::MatrixXd &curve, const Eigen::MatrixXd &points, int i) {
    double energy = 0.0;
    ...
    for (int j = 0; j < points.rows(); ++j) {
        double dist = (curve.row(i) - points.row(j)).squaredNorm();
        if (dist < min_dist) {
            min_dist = dist;
        }
    }
...
void optimize_snake(GUIParams& params, Eigen::MatrixXd &resampled_points, const Eigen::MatrixXd &points) {
    for (int iter = 0; iter < params.snake_iteration_num; ++iter) {
        ...
        for (int i = 0; i < resampled_points.rows(); ++i) { // Exclude endpoints
            ...
            for (int dim = 0; dim < 3; ++dim) {
                ...
                double energy_plus = compute_energy(params, test_curve, points, i);
                
                test_curve(i, dim) -= 0.02;
                double energy_minus = compute_energy(params, test_curve, points, i);
